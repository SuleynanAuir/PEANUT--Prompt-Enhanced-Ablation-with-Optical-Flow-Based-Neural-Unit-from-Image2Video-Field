...
class GenerateSegmentIndices:
    ...
    def __call__(self, results):
        ...
        # DEBUG: Print path construction details
        print(f"[GenerateSegmentIndices DEBUG]")
        print(f"  lq_path_root: {repr(lq_path_root)}")
        print(f"  clip_name: {repr(clip_name)}")
        print(f"  filename_tmpl: {repr(self.filename_tmpl)}")
        print(f"  neighbor_list[0]: {neighbor_list[0] if neighbor_list else 'N/A'}")
        ...
        # DEBUG: Print first constructed path
        if lq_path:
            print(f"  First lq_path: {repr(lq_path[0])}")
